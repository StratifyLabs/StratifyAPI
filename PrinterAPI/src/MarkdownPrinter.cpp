...
MarkdownPrinter &
MarkdownPrinter::open_header(const var::StringView &header, Level level) {
  m_is_last_close = false;
  ...
  for (u32 row = 0; row < m_pretty_table.count(); row++) {
    ...
    *this << "|";
    for (u32 column = 0; column < m_pretty_table.at(row).count(); column++) {
      const var::StringView &cell = m_pretty_table.at(row).at(column);
