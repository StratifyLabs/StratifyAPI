...
Printer &Printer::key(const var::StringView &key, bool value) {
  print(Level::info, key.cstring(), value ? "true" : "false");
  return *this;
}

Printer &Printer::key(const var::StringView &key, const var::StringView &a) {
  print(Level::info, key.cstring(), a.cstring());
  return *this;
}

Printer &Printer::key(const var::StringView &key, const char *fmt, ...) {
  va_list list;
  va_start(list, fmt);
...
Printer &Printer::error(const api::Result result, u32 line_number) {
  error(
...
Printer &
Printer::trace(const char *function, int line, const var::StringView &message) {
