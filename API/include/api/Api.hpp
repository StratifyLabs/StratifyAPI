...
/*! \mainpage
 *
 * <small>This is the <b>Stratify Application Programming Interface (API)</b>
 *documentation.  This documentation is for building applications for Stratify
 *OS. If you are creating board support packages, you will want to take a look
 *at the <a href="../../StratifyOS/html/">Stratify OS documentation</a>.</small>
 *
 * \section Introduction
 *
 * StratifyAPI is a library for Stratify OS applications that makes it easy to
 *access hardware and other system functions (you can also use standard \ref
 *POSIX and \ref STDC functions if that is your style).
 *
 * If you are new, it would be highly beneficial to quickly read through the
 *naming conventions:
 * https://github.com/StratifyLabs/StratifyLib#naming-conventions. Once you are
 *familiar with those, you can check out these useful namespaces to get started.
 *
 ...
 *
 * The following examples will give you an idea about how things work. These are
 *the objects used in the examples.
 *
 ...
 *
 * \code
 * #include <sapi/hal.hpp> //this will include all namespace classes and add the
 *namespace i.e. "using namespace hal;" #include <sapi/sys.hpp> //includes all
 *sys classes plus "using namespace sys;" #include <sapi/chrono.hpp> //includes
 *all chrono classes plus "using namespace chrono;"
 *
 * int main(int argc, char * argv[]){
 *  Pin p(2, 10);
 *  Pin button(2,9);
 *  p.init(Pin::FLAG_SET_OUTPUT); //open the port and configure pin 10 as an
 *output button.init(Pin::FLAG_SET_INPUT | Pin::FLAG_IS_PULLUP); //pin 9 is an
 *input (pulled high)
 *
 ...
 *https://github.com/StratifyLabs/HelloWorld#helloworld.
 *
 */
...
enum error_codes : s32 {
  ...
  error_code_inet_failed_to_create_socket /*! Failed to create a socket (1) */
  = -(error_code_flag_inet | 1),
  error_code_inet_failed_to_connect_to_socket /*! Failed to connect to socket
                                                 (2) */
  = -(error_code_flag_inet | 2),
  error_code_inet_failed_to_write_header /*! Failed to write request header (3)
                                          */
  = -(error_code_flag_inet | 3),
  error_code_inet_failed_to_write_data /*! Failed to write data (4) */ = -(
    error_code_flag_inet | 4),
  error_code_inet_failed_to_write_incoming_data_to_file /*! Failed  write
                                                           incoming data to file
                                                           provided (5) */
  = -(error_code_flag_inet | 5),
  error_code_inet_failed_to_find_address /*! Failed to find IP address of URL
                                            (6) */
  = -(error_code_flag_inet | 6),
  error_code_inet_failed_to_get_status_code /*! Failed to get a status code in
                                               the HTTP response (7) */
  = -(error_code_flag_inet | 7),
  error_code_inet_failed_to_get_header /*! Failed to receive the header (8) */
  = -(error_code_flag_inet | 8),
  error_code_inet_failed_wrong_domain = -(error_code_flag_inet | 9),
  error_code_inet_wifi_api_missing = -(error_code_flag_inet | 10),
  error_code_inet_failed_to_write_incoming_chunked_data_to_file /*! Failed
                                                                   write
                                                                   incoming data
                                                                   to file
                                                                   provided (5)
                                                                 */
  = -(error_code_flag_inet | 11),
