...
public:
  ...
  using Environment
    = arg::Argument<const var::String &, struct SysEnvironmentTag>;
  using DestinationPath
    = arg::Argument<var::String &, struct SysEnvironmentTag>;
   ...
   *
   * This method must be called locally in an app. It can't be executed over the
   * link protocol.
   */
  static int launch(
   ...
   *
   * @param path The path to the application
   * @param exec_dest A pointer to a buffer where the execution path will be
   * written (null if not needed)
   * @param args The arguments to pass to the applications
   * @param options For example:  LAUNCH_OPTIONS_FLASH | LAUNCH_OPTIONS_STARTUP
   * @param ram_size The amount of RAM that will be used by the app (0 assumes
   * default value)
   * @param update_progress A callback to show the progress if the app needs to
   * be installed (copied to flash/RAM)
   ...
   */
  static void
  powerdown(const chrono::MicroTime &duration = chrono::Milliseconds(0));
   ...
   */
  static int
  hibernate(const chrono::MicroTime &duration = chrono::Milliseconds(0));
   ...
   */
  static int
  request(KernelRequest req, KernelArgument argument = KernelArgument(nullptr));
...
class Printer;
Printer &operator<<(Printer &printer, const SysInfo &a);
class TraceEvent;
