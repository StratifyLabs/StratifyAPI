...
public:
   ...
   *
   */
  explicit Microseconds(u32 microseconds = 0) {
  ...
  bool operator==(const Microseconds &a) const {
...
public:
  ...
  Time(
    const var::StringView &time_string,
    const var::StringView &format = "%Y-%m-%d %H:%M:%S");
  ...
  static Time from_minutes(u32 value) {
    return Time(Seconds(0), Minutes(value), Hours(0));
  }
  static Time from_hours(u32 value) {
    return Time(Seconds(0), Minutes(0), Hours(value));
  }
  static Time from_days(u32 value) {
    return Time(Seconds(0), Minutes(0), Hours(value * 24));
  }
  static Time from_weeks(u32 value) {
    return Time(Seconds(0), Minutes(0), Hours(value * 24 * 7));
  }
  ...
  Time age() const { return Time::get_time_of_day() - *this; }
...
void wait(const Microseconds &duration);
