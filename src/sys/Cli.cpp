...
Cli::Cli(int argc, char *argv[], const char *app_git_hash)
...
bool Cli::compare_with_prefix(
  ...
  const var::String &argument) const {
  String with_prefix;
  if (argument.at(0) != '-') {
    return false;
  }
  ...
  if (with_prefix.create_sub_string(String::Position(1)) == argument) {
    return true;
  }
  return false;
}
...
var::String Cli::get_option(const String &name, Description help) const {
  var::String result;
  u32 args;

  if (help.argument().is_empty() == false) {
    m_help_list.push_back(String() << name << ": " << help.argument());
  }
...
mcu_pin_t Cli::get_option_pin(const char *option) const {
  mcu_pin_t pio;
  Tokenizer arg(option, var::Tokenizer::Delimeters("."));
...
mcu_pin_t Cli::pin_at(u16 value) const {
  mcu_pin_t pio;
  Tokenizer arg(at(value), var::Tokenizer::Delimeters("."));
...
int Cli::value_at(u16 value) const { return at(value).to_integer(); }
...
bool Cli::handle_uart(hal::UartAttributes &attr) const {
  if (is_option("-uart")) {
    ...
    if (is_option("-tx")) {
      attr.set_tx(get_option_pin("-tx"));
    }
    if (is_option("-rx")) {
      attr.set_rx(get_option_pin("-rx"));
    }
    if (is_option("-rts")) {
      attr.set_rts(get_option_pin("-rts"));
    }
    if (is_option("-cts")) {
      attr.set_cts(get_option_pin("-cts"));
    }
...
bool Cli::handle_i2c(hal::I2CAttr &attr) const {
  if (is_option("-i2c")) {
    ...
    if (is_option("-slave")) {
      attr.set_slave_addr(get_option_hex_value("-slave"));
    }
    if (is_option("-pu")) {
      o_flags |= hal::I2C::flag_is_pullup;
    }

    if (is_option("-sda")) {
      attr.set_sda(get_option_pin("-sda"));
    }
    if (is_option("-scl")) {
      attr.set_scl(get_option_pin("-scl"));
    }
