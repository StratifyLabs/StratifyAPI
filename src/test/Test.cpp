...
Test::Test(const var::String &name, Test *parent) {
  ...
  if (!m_is_initialized) {
    initialize(Name("unknown"), VersionEncodedString("0.0"));
  }
...
void Test::print_case_message_with_key(
...
void Test::initialize(const Options &options) {
  ...
  if ((sys.open() < 0) || (sys.get_info(info) < 0)) {
    ...
    print_indent(
      2,
      "\"serialNumber\": \"%08lX%08lX%08lX%08lX\",\n",
      info.serial.sn[3],
      info.serial.sn[2],
      info.serial.sn[1],
      info.serial.sn[0]);
...
u32 Test::parse_execution_flags(const sys::Cli &cli) {
  ...
  if (
    cli.get_option(
      "all",
      Cli::Description("execute all tests and types (if no type "
  ...
  if (cli.get_option("api", Cli::Description("execute api tests")) == "true") {
    o_execute_flags |= Test::execute_api;
  }

  if (
    cli.get_option("performance", Cli::Description("execute performance tests"))
    == "true") {
    o_execute_flags |= Test::execute_performance;
  }

  if (
    cli.get_option("stress", Cli::Description("execute stress test"))
    == "true") {
    o_execute_flags |= Test::execute_stress;
  }

  if (
    cli.get_option("additional", Cli::Description("execute additional test"))
    == "true") {
...
u32 Test::parse_test(
  ...
  if (
    cli.get_option(
      name,
      Cli::Description(var::String("Execute the ") + name + " test suite"))
    == "true") {
    return test_flag;
  }
