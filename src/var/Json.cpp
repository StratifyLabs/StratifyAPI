...
JsonObject &JsonObject::update(const JsonValue &value, enum updates o_flags) {
  ...
  set_error_number_if_error(api()->object_update(m_value, value.m_value));
  return *this;
}
...
JsonArray &JsonArray::append(const JsonValue &value) {
  ...
  set_error_number_if_error(api()->array_append(m_value, value.m_value));
  return *this;
}
...
JsonArray &JsonArray::append(const JsonArray &array) {
  ...
  set_error_number_if_error(api()->array_extend(m_value, array.m_value));
  return *this;
}
...
JsonArray &JsonArray::remove(size_t position) {
  set_error_number_if_error(api()->array_remove(m_value, position));
  return *this;
}

JsonArray &JsonArray::clear() {
  set_error_number_if_error(api()->array_clear(m_value));
  return *this;
}
...
JsonValue JsonDocument::load(FilePath path) {
  JsonValue value;
...
JsonValue JsonDocument::load(const var::String &json) {
  if (json.length() > 0) {
    ...
    if (start_char == '"') {
      return JsonString(json.create_sub_string(
        String::Position(1),
...
size_t JsonDocument::load_file_data(void *buffer, size_t buflen, void *data) {
  ...
  return f->read(buffer, fs::File::Size(buflen));
}
...
int JsonDocument::save(const JsonValue &value, fs::File::Path path) const {
  fs::File f;
  int result;
  ...
  result = JsonValue::api()->dump_file(
    ...
    flags());
#else
  if (f.create(path.argument(), fs::File::IsOverwrite(true)) < 0) {
...
var::String JsonDocument::to_string(const var::JsonValue &value) const {
  ...
  if (
    JsonValue::api()
      ->dumpb(value.m_value, result.to_char(), result.capacity(), flags())
    == 0) {
    return var::String();
  }
  return result;
...
int JsonDocument::save(const JsonValue &value, fs::File &file) const {
...
int JsonDocument::save(
  ...
  void *context) const {
  return JsonValue::api()
    ->dump_callback(value.m_value, callback, context, flags());
}
