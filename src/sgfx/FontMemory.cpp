...
void FontMemory::set_font_memory(const void * ptr){
	...
	if( m_font != 0 ){
		...
		memcpy(&m_hdr, hdr_ptr, sizeof(sg_font_hdr_t));
...
int FontMemory::load_kerning(u16 first, u16 second) const {
	...
	const sg_font_kerning_pair_t * pairs = (const sg_font_kerning_pair_t *)((const char *)m_font + sizeof(sg_font_hdr_t));
	int i;
...
int FontMemory::load_char(sg_font_char_t & ch, char c, bool ascii) const {
	...
	int ind;
	sg_font_char_t * chp;
	const sg_font_hdr_t * hdr = (const sg_font_hdr_t *)m_font;
	...
	//header has sg_font_hdr_t then kerning pairs then char indices
	offset = sizeof(sg_font_hdr_t) + sizeof(sg_font_kerning_pair_t)*hdr->kerning_pairs + ind*sizeof(sg_font_char_t);
