...
void Image::draw_to_scale(const DrawingScaledAttr & attr){
	...
	if( m_bitmap != 0 ){
		...
		if( is_align_left() ){
			...
			top_left.x = attr.point().x + attr.dim().width - m_bitmap->width();
		} else {
			top_left.x = attr.point().x + attr.dim().width/2 - m_bitmap->width()/2;
		}

		if( is_align_top() ){
			top_left.y = attr.point().y;
		} else if( is_align_bottom() ){
			top_left.y = attr.point().y + attr.dim().height - m_bitmap->height();
		} else {
			top_left.y = attr.point().y + attr.dim().height/2 - m_bitmap->height()/2;
		}
