...
void sg_pour(sg_bmap_t * mg, sg_point_t p){
	...
	sg_icon_primitive_tst_hedge(mg, p, &xmin, &xmax); //find the bounding points xmin and xmax
	is_above = !sg_icon_primitive_tst_hline(mg, xmin, xmax, p.y+1, &(above.x)); //see if anywhere above the bounded region is blank
	is_below = !sg_icon_primitive_tst_hline(mg, xmin, xmax, p.y-1, &(below.x)); //see if anywhere below the bounded region is blank
...
u8 sg_icon_primitive_tst_hline(const sg_bmap_t * mg, sg_int_t xmin, sg_int_t xmax, sg_int_t y, sg_int_t * pos){
	sg_point_t p;
	...
	for(p.x = xmin; p.x < xmax; p.x++){
		if( sg_icon_primitive_tst_bounded_pixel(mg,p) == 0 ){
			*pos = p.x;
			return false;
		}
	}
...
void sg_icon_primitive_tst_hedge(const sg_bmap_t * mg, sg_point_t p, sg_int_t * xmin, sg_int_t * xmax){
	sg_point_t min;
	sg_point_t max;
	sg_bound(mg, &p);
	min.point = p.point;
	max.point = p.point;
	while( sg_icon_primitive_tst_bounded_pixel(mg, min) == false ){ min.x--; }
	while( sg_icon_primitive_tst_bounded_pixel(mg, max) == false ){ max.x++; }
	*xmin = min.x+1;
	*xmax = max.x;
	return;
}
...
u8 sg_icon_primitive_tst_bounded_pixel(const sg_bmap_t * mg, sg_point_t p){
...
u8 sg_icon_primitive_tst_pixel(const sg_bmap_t * mg, sg_point_t p){
	if( sg_point_visible(mg, p) ){
		return sg_icon_primitive_tst_bounded_pixel(mg,p);
	}
	return 1;
}
