...
int Dir::remove(
  ...
  IsRecursive recursive SAPI_LINK_DRIVER_LAST) {
  int ret = 0;
  ...
  if (ret >= 0) {
    ...
    if (link_driver.argument()) {
      ret = File::remove(path, link_driver);
    } else {
...
int Dir::copy(
  ...
  var::Vector<var::String> source_contents = Dir::read_list(
  ...
  for (auto entry : source_contents.vector()) {
    var::String entry_path = var::String()
                             << source_path.argument() << "/" << entry;

    var::String destination_entry_path
      = var::String() << destination_path.argument() << "/" << entry;
...
int Dir::create(
  const var::String &path,
  const Permissions &permissions SAPI_LINK_DRIVER_LAST) {
  int result;
...
int Dir::create(
  ...
  Recursive is_recursive SAPI_LINK_DRIVER_LAST) {
  ...
  var::Tokenizer path_tokens(path, var::Tokenizer::Delimeters("/"));
...
bool Dir::exists(const var::String &path SAPI_LINK_DRIVER_LAST) {
  Dir d
#if defined __link
    (link_driver)
#endif
...
var::Vector<var::String> Dir::read_list(
  const var::String &path,
  IsRecursive is_recursive SAPI_LINK_DRIVER_LAST) {
  return read_list(
    path,
    nullptr,
    is_recursive
    ...
    if (!entry.is_empty() && (entry != ".") && (entry != "..")) {
      ...
      if (is_recursive.argument()) {
        ...
        if (info.is_directory()) {
          ...
          var::Vector<var::String> intermediate_result = Dir::read_list(
            entry_path,
            filter,
            is_recursive
#if defined __link
...
var::Vector<var::String> Dir::read_list(const IsRecursive is_recursive) {
  return read_list(
