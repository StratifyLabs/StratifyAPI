...
public:
  ...
  LinkPath(const var::String &path, link_transport_mdriver_t *driver) {
    if (path.find(host_prefix()) == 0) {
      m_driver = nullptr;
      m_path
        = path.create_sub_string(var::String::Position(host_prefix().length()));
    } else {
      m_driver = driver;
      size_t position;
      ...
      m_path = path.create_sub_string(var::String::Position(position));
    }
  }
...
public:
  ...
  using SourceLinkPath
    = arg::Argument<const LinkPath &, struct LinkSourceLinkPathTag>;
  using DestinationLinkPath
    = arg::Argument<const LinkPath &, struct LinkDestinationLinkPathPathTag>;
  using ApplicationName
    = arg::Argument<const var::String &, struct LinkApplicationNameTag>;
  using IsLegacy = arg::Argument<bool, struct LinkIsLegacyTag>;
  using RetryCount = arg::Argument<u32, struct LinkRetryCountTag>;
  using RetryDelay
    = arg::Argument<const chrono::MicroTime &, struct LinkRetryDelayTag>;
  ...
  using BootloaderRetryCount
    = arg::Argument<u32, struct LinkBootloaderRetryCountTag>;
   ...
   *
   */
  int connect(const var::String &path, IsLegacy is_legacy = IsLegacy(false));
   ...
   * \note Stratify OS does not currently support symbolic links.  This function
  ...
  int readdir_r(
    ...
    struct link_dirent *
      *result /*! Assigned to \a entry on success or NULL on failure */
  );
  ...
  int copy(
    ...
    const fs::Permissions
      &permissions /*! The access permissions if copying to the device */,
    IsCopyToDevice to_device
    = IsCopyToDevice(true) /*! When true, copy is from host to device */,
  ...
  static bool directory_exists(const LinkPath &link_path) {
    return fs::Dir::exists(
      link_path.path(),
      fs::Dir::LinkDriver(link_path.driver()));
  }
  ...
  int open(
    ...
    const fs::Permissions &permissions
    = fs::Permissions(0) /*! The access permissions when creating a new file */
  );
   ...
   */
  int read(FileDescriptor fd, void *buf, fs::File::Size nbyte);
   ...
   */
  int write(FileDescriptor fd, const void *buf, fs::File::Size nbyte);
   ...
   * \return Zero on success
   */
  int lseek(FileDescriptor fd, fs::File::Location offset, int whence);
   ...
   * \return Zero on success
   */
  int rename(SourcePath old_path, DestinationPath new_path);
   ...
   * \note Ownership is not supported on all filesystems.
   */
  int chmod(const var::String &path, const fs::Permissions &permissions);
   ...
   * @param update A callback to update the caller on the progress of the
   * install
  ...
  const var::String &serial_number() const {
    return m_link_info.serial_number();
  }
  ...
  /*! \details The path of the currently connected (or last connected) notify
   * device */
