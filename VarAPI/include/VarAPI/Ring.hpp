...
public:
  ...
  T &at(size_t position) {
    ...
    return Item(*this).at<T>(offset % count());
    // return Item(*this).at<T>(offset % count());
  }
  ...
  T &back() {
    if (m_tail == m_count) {
      return Item(*this).at<T>(m_head);
    }
    return Item(*this).at<T>(m_tail);
  }
  ...
  T &front() {
    if (m_head) {
      return Item(*this).at<T>(m_head - 1);
    }
    return Item(*this).at<T>(m_count - 1);
  }
  ...
  Data to_linear_data() const {
    ...
    for (u32 i = 0; i < count(); i++) {
      new ((void *)(Item(result).to<T>() + i)) T(at(i));
    }
    return result;
  }
