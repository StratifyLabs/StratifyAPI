...
public:
  ...
  using CreateSubStringOptions = EraseOptions;
  ...
  explicit String(const StringView &s) : m_string(s.string_view()) {}
  ...
  explicit String(const var::Item &item);
  ...
  String &operator+=(const StringView &a) { return append(a); }
  ...
  String &erase(const StringView &string_to_erase, size_t position = 0);
  ...
  size_t count(const var::StringView &to_count) const;
  ...
  String &append(const StringView &string_to_append) {
  ...
  Vector<String> split(const StringView &delimiter) const;
...
String operator+(const StringView &lhs, const char *&rhs);
