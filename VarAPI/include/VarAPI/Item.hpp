...
public:
  ...
  template <typename T> Item(const Vector<T> &vector) {
    set_reference(Options()
  ...
  template <typename T> Item(Vector<T> &vector) {
    set_reference(Options()
                    .set_read_buffer(vector.to_const_void())
  ...
  Item(const Array<T, size_value> &array) {
    set_reference(Options()
  ...
  template <typename T, size_t size_value> Item(Array<T, size_value> &array) {
    set_reference(Options()
                    .set_read_buffer(array.to_const_void())
  ...
  template <typename T> explicit Item(T &item) {
    // catch all
    refer_to(item);
  }
  ...
  Item &refer_to(const Item &reference) {
   ...
   *
   */
  template <typename T> Item &refer_to(T &item) {
   ...
   *
   */
  Item &refer_to(const Options &options) {
    set_reference(options);
    return *this;
  }
   ...
   *
   */
  template <typename T> Item &fill(const T &value, size_t count = 0) {
    if (count == 0) {
      count = this->count<T>();
    }
   ...
   *
   */
  Item &swap_byte_order(SwapBy order);

  /*! \details Returns true if the data object is read only.
   *
   */
  bool is_read_only() const { return m_data_write == nullptr; }

  /*! \details Returns true if the contents
   * of both Item objects are the same.
   *
   */
  ...
  ssize_t size_signed() const { return static_cast<ssize_t>(m_size); }
  ...
  Item &copy(const Item &source) {
    if (m_data_write) {
      size_t copy_size = size() > source.size() ? source.size() : size();
      memcpy(m_data_write, source.to_const_void(), copy_size);
    } else {
   ...
   * //md2code:main
   * char buffer[64];
   * Item a(buffer); //allocate 64 bytes of data
   ...
   * ```
   */
  template <typename T> u32 count() const { return size() / sizeof(T); }
...
protected:
  ...
  const void *read_data() const { return m_data_read; }

  void *write_data() const { return m_data_write; }

  void set_reference(const Options &options);

private:
  const void *m_data_read = nullptr;
  void *m_data_write = nullptr;
  size_t m_size = 0;
...
namespace sys {
class Printer;
Printer &operator<<(Printer &printer, const var::Item &a);
} // namespace sys
#endif
