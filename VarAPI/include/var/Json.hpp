...
public:
  ...
  JsonValue &copy(const JsonValue &value, IsDeep is_deep = IsDeep(true));
...
public:
   ...
   *
   */
  JsonDocument(u32 o_flags = option_indent3) { set_flags(o_flags); }

  JsonDocument &set_options(u32 o_option) {
    m_flags = o_option;
    return *this;
  }
  u32 options() const { return m_flags; }

  JsonDocument &set_flags(u32 flags) {
    m_flags = flags;
    return *this;
  }
   ...
   * \return
   */
  JsonValue load(const String &json);

#if defined __link
  using XmlString
    = arg::Argument<const String &, struct JsonDocumentXmlStringTag>;
  using XmlFilePath
    = arg::Argument<const String &, struct JsonDocumentXmlFilePathTag>;

  using XmlIsFlat = arg::Argument<bool, struct JsonDocumentXmlIsFlathTag>;
   ...
   * \return
   */
  JsonValue load(XmlString xml, XmlIsFlat is_flat = XmlIsFlat(true));

  JsonValue load(XmlFilePath path, XmlIsFlat is_flat = XmlIsFlat(true));
#endif
  ...
  int save(const var::JsonValue &value, fs::File::Path path) const;
  ...
  int save(const JsonValue &value, fs::File &file) const;

  int save(const JsonValue &value, json_dump_callback_t callback, void *context)
    const;
