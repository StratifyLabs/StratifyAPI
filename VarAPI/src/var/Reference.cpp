...
Reference::Reference() { set_size_internally(0); }
...
Reference::Reference(ReadOnlyBuffer buffer, Size size) {
  refer_to(buffer, size);
}

Reference::Reference(ReadWriteBuffer buffer, Size size) {
  refer_to(buffer, size);
}

Reference::Reference(void *buffer, Size size) { refer_to(buffer, size); }

Reference &Reference::refer_to(ReadOnlyBuffer read_only_data, Size size) {
  set_reference(read_only_data, ReadWriteBuffer(nullptr), size);
  return *this;
}

Reference &Reference::refer_to(void *data, Size size) {
  set_reference(ReadOnlyBuffer(data), ReadWriteBuffer(data), size);
  return *this;
}
...
int Reference::save(const String &file_path, IsOverwrite is_overwrite) const {
  fs::File f;
  if (f.create(file_path, is_overwrite) < 0) {
    return -1;
  }
