...
Data &Data::copy_contents(const Item &item) {
  return copy_contents(item, CopyContentsOptions().set_size(item.size()));
}

Data &Data::copy_contents(const Item &item, size_t size) {
  return copy_contents(item, CopyContentsOptions().set_size(size));
}

Data &Data::copy_contents(const Item &a, const CopyContentsOptions &options) {
...
Data &Data::append(const Item &item) {
...
Data &Data::operator<<(const Item &item) { return append(item); }
